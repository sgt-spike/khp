      </main>
      <footer class="footer">
         <div id="social" class="social">
            <a href="http://www.facebook.com/765813026943799"><i class="fab fa-facebook-f"></i></a>
            <a href="https://www.linkedin.com/in/katie-s-handprint-4b8a23161"><i class="fab fa-linkedin-in"></i></a>
            <a href="https://twitter.com/KatiesHandprint"><i class="fab fa-twitter"></i></a>
            <a href="http://instagram.com/katieshandprint"><i class="fab fa-instagram"></i></a>
         </div>
         <div id="address" class="info">
            <p>4161 Mariah Drive <br>
               Eagle River, AK 99577 <br>
               (907) 770-1000<br>
               Email: <a href="mailto:admin@katiesHandprint.org">Admin@katiesHandprint.org</a>
            </p>
         </div>
         <div id="dev" class="info">   
            <p>Developed by <a href="https://www.spikedevelopments.com">Spike Developments</a></p>
         </div>
         <div id="copy" class="info">
            <p id="admin">Katie's Handprint &copy; 2018</p>
         </div>
      </footer>
      <script src="js/navsidebar.js"></script>
      <script src="js/admin.js"></script>
      <script src="js/links.js"></script>
   </div>
</body>
</html>